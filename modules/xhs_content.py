...
- 末尾互动：「哪条最炸？评论区聊 👇」
- **全文硬性限制≤800字，超出会被截断，宁短勿长**
- 每条1-2句就够了，格式：「⭐ 标题 核心提炼。💬 锐评。」
...
{{
  ...
  "body": "小红书正文（≤800字，每条1-2句提炼+锐评，不多写一个字）",
...
## 2. 详细解读 (detail_zh) — 350-500字
- 深度展开但自然克制：事件背景 → 核心数据/技术细节 → 行业影响
- 信息密度高但不堆砌，像资深分析师在给朋友讲解
- 必须包含：至少1个具体数字、至少1个关键公司/人物名称
- 字数不够不要硬凑，字数超标自然收尾不要突然截断
...
[
  {{
    ...
    "detail_zh": "350-500字详细解读，自然流畅不硬凑，每句话有信息增量，在合适处自然结尾。例如：DeepMind今日在Nature以封面文章发表AlphaFold 3，所有生命分子三维结构预测精度达RMSD小于1.0埃。辉瑞CEO透露肿瘤管线周期从4.5年压缩至18个月，诺华罗氏默克等巨头已签约。Isomorphic Labs已基于该技术设计出3款候选分子。北京智源研究院联合清华北大启动中国版项目。Nature社论称这是十年来结构生物学最重要突破。"
  }},
  ...
]"""
